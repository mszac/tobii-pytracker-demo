...
run_block() {
  ...
  python examples/timeseries_noise_demo/validate_collection.py \
    --domain "$domain" \
    --output-root output/timeseries_noise_demo \
    --started-at "$started_at"
}

run_block machine examples/timeseries_noise_demo/config.machine.native.yaml "MACHINE VIBRATION"
run_block pv examples/timeseries_noise_demo/config.pv.native.yaml "PV POWER"
