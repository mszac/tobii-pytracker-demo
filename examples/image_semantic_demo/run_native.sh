...
python examples/image_semantic_demo/run_experiment.py \
  --config-file examples/image_semantic_demo/config.native.yaml \
  --eyetracker-config-file ../configs/mouse_eyetracker_config.yaml \
  --loop-count 12

python examples/image_semantic_demo/validate_collection.py \
  --output-root output/image_semantic_demo \
  --started-at "$STARTED_AT"

echo "NATIVE_IMAGE_SEMANTIC_COLLECTION_COMPLETE"
