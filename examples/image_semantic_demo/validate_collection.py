...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-root", default="output/image_semantic_demo")
    p.add_argument("--manifest", default="examples/image_semantic_demo/stimuli_manifest.csv")
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if response not in {"biological", "object", "scene"}:
    ...
    presented = [str(row["classification"]).strip().lower() for row in rows]
    if any(a == b for a, b in zip(presented, presented[1:])):
        raise RuntimeError(f"Image categories are not sufficiently mixed: adjacent repeat in {presented}")
    for start in range(0, EXPECTED_TRIALS, 3):
        window = presented[start:start + 3]
        if set(window) != set(EXPECTED_CLASSES):
            raise RuntimeError(f"Trials {start + 1}-{start + 3} are not balanced across categories: {window}")
