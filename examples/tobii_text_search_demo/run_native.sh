...
tobii-pytracker \
  --config_file examples/tobii_text_search_demo/config.native.yaml \
  --eyetracker_config_file ../configs/mouse_eyetracker_config.yaml \
  --enable_eyetracker \
  --loop_count 12

python examples/tobii_text_search_demo/validate_collection.py \
  --output-root output/tobii_text_search_demo \
  --started-at "$STARTED_AT"

echo "NATIVE_TEXT_SEARCH_COLLECTION_COMPLETE"
