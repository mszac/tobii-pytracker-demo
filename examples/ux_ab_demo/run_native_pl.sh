...
python examples/ux_ab_demo/run_experiment.py \
  --config-file examples/ux_ab_demo/config.native_pl.yaml \
...
python examples/ux_ab_demo/validate_collection.py \
  --output-root output/ux_ab_demo_pl \
  --dataset examples/ux_ab_demo/data/text_search_pl.csv \
  --started-at "$STARTED_AT"
echo "NATIVE_UX_AB_COLLECTION_COMPLETE_PL"
